...
def calculate_nodes_coords(graph, root, current_node, node_coordinates, parent_list, triplets, discovery_time,
    ...
    # Sort the children such that the ones with a greater subgraph will be the first ones picked
    sorted_neighbors = sorted(graph[current_node], key=lambda x: subtree_sizes[x], reverse=True)
    for neighbor in sorted_neighbors:
        if neighbor not in visited:
            calculate_nodes_coords(graph, root, neighbor, node_coordinates, parent_list, triplets,
                                   discovery_time, visited, subtree_sizes)
